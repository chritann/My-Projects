...
void batteryICI2C(uint8_t DATA1, uint8_t DATA2){
  ...
  if ((TWSR & 0xF8) != START) ERROR2();  //If START condition not received, error
  ...
  if ((TWSR & 0xF8) != 0x18) ERROR2();  //ERROR if SLA+W not sent and acknowledged
  ...
  if((TWSR & 0xF8) != 0x28) ERROR2();  //ERROR if SDA
 ...
 * returns: register 
 */
uint8_t I2C_receive_batteryIC(uint8_t data2){  
  ...
  if((TWSR & 0xF8) != 0x10) ERROR2();  //Error for repeat start condition
  ...
  if((TWSR & 0xF8) != 0x40) ERROR2();  // checks if SLA+R and ACK has been sent
